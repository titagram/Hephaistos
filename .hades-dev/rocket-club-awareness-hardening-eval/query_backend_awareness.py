...
def main() -> int:
    ...
    out = EVAL_ROOT / "reports" / "backend-awareness-after-candidates.json"
    out.write_text(json.dumps(awareness, indent=2, sort_keys=True), encoding="utf-8")
    print(
        json.dumps(
            {
                "diagnosable": awareness.get("diagnosable_without_source"),
                "coverage": awareness.get("coverage"),
                "actions": awareness.get("actions"),
            },
            sort_keys=True,
        )
    )
