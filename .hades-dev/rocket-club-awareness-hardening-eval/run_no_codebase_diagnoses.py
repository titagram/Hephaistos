...
def _fixtures_for_model(model_family: str, fixture_path: Path = FIXTURE_PATH) -> list[dict[str, Any]]:
...
Expected root_cause_id for complete cases: {fixture.get("expected_root_cause_id")}
...
6. Call hades_backend_causal_pack_fetch with query/root_cause_id for complete
   cases, or with the case query for insufficient cases to prove no replayable
   causal pack supports a precise claim.
7. Call hades_backend_diagnosis_report_create to persist the final diagnosis.
...
- causal packs as causal_pack:<id> in causal_pack_refs for precise diagnoses
- causal_chain entries that include the key bug_evidence/evidence_pack/graph/
  source_slice refs and diagnosis:<root_cause_id>
The safest causal_chain format is a flat array of strings. For complete cases,
include every evidence_refs item that directly supports the mechanism, plus
diagnosis:<root_cause_id>. Do not omit graph refs from causal_chain when they
are listed in evidence_refs.
...
must be "current", awareness.diagnosable_without_source must be true, and
causal_pack_refs must include a replayable pack returned by
hades_backend_causal_pack_fetch. Pass the same causal_pack_refs to
hades_backend_diagnosis_report_create.
If the fixture has an expected_root_cause_id and hades_backend_causal_pack_fetch
returns a causal pack with status "valid" for that root_cause_id, do not
downgrade confidence solely because unrelated graph search candidates are noisy
or because a replay response contains optional/missing logical graph refs. Use
the fixture confidence guidance and the returned pack id.
For insufficient fixtures, you must still call hades_backend_causal_pack_fetch
and the final JSON must set counterfactual_refused true when you refuse the
precise claim because source_slice evidence is missing.
...
diagnosable_without_source, evidence_refs, causal_pack_refs, causal_chain,
counterfactual_refused, missing_evidence, mechanism.
""".strip()
...
def main() -> int:
    ...
    fixtures = _fixtures_for_model(args.model_family, Path(args.fixture_path).expanduser())
    if args.fixture_id:
