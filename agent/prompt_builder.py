...
DEFAULT_AGENT_IDENTITY = (
    "You are Hades Agent, an intelligent AI assistant created for Hades. "
...
HERMES_AGENT_HELP_GUIDANCE = (
    "You run on Hades Agent. When the user needs help with "
    "Hermes itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "the documentation at https://hades-agent.local/docs is your "
...
KANBAN_GUIDANCE = (
    ...
    "Exception: standalone code changes with no downstream reviewer card should "
    "end with `kanban_block(reason=\"review-required: ...\")`. **Managed OrgRun "
    "nodes use their materialized gates instead:** an execution node with a reviewer "
    "child must `kanban_complete` the finished candidate; do not use review-required, "
    "or the reviewer can never start. A managed review must call `kanban_complete` "
    "with `metadata.review.verdict` set to `pass` or `fail`. `pass` advances the DAG; "
    "`fail` atomically records changes requested, reopens the upstream execution or "
    "integration node, and leaves the reviewer waiting for the next candidate. "
    "Actionable findings are remediation work, not a human blocker.\n"
