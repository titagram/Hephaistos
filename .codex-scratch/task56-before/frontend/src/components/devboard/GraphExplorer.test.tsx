...
function envelope(type: any, values: any = {}): DashboardGraphResponse {
  return {
    ...
    scope, projection, node: null, items: [], edges: [], returned: 0, limit: 50,
...
async function mount(queryGraph: any = jest.fn(successfulQuery)) {
  await act(async () => {
    root.render(<MemoryRouter><GraphExplorer projectId="project-1" scopes={scopes} queryGraph={queryGraph} /></MemoryRouter>);
  });
  await settle();
  return queryGraph;
}
...
describe("GraphExplorer", () => {
  ...
  it("uses the chosen scope and opaque handle for search, detail, parallel neighborhoods, impact and path", async () => {
    ...
    await act(async () => { input("Search symbols").value = "InvoiceService"; input("Search symbols").dispatchEvent(new Event("input", { bubbles: true })); });
    await settle(300);
  ...
  it("renders empty, unavailable and retryable error states while preserving the selected handle", async () => {
    ...
    await act(async () => { input("Search symbols").value = "Missing"; input("Search symbols").dispatchEvent(new Event("input", { bubbles: true })); });
    await settle(300);
    expect(container.textContent).toContain("No matching symbols");

    act(() => root.unmount()); root = createRoot(container);
    await act(async () => { root.render(<MemoryRouter><GraphExplorer projectId="project-1" scopes={scopes} projectionUnavailable queryGraph={emptyQuery} /></MemoryRouter>); });
    ...
    expect(window.location.search).toContain("symbol=opaque-invoice");
