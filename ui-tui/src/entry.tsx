...
import type { FrameEvent } from '@hades/ink'
...
const dumpNotice = (snap: MemorySnapshot, dump: HeapDumpResult | null) =>
  `hades-tui: ${snap.level} memory (${formatBytes(snap.heapUsed)}) — auto heap dump → ${dump?.heapPath ?? dump?.diagPath ?? '(failed)'}\n`
...
setupGracefulExit({
  ...
  onError: (scope, err) => {
    ...
    process.stderr.write(`hades-tui lifecycle ${scope}: ${message.slice(0, 2000)}\n`)
  },
  onSignal: signal => {
    ...
    resetTerminalModes()
    process.stderr.write(`hades-tui lifecycle: received ${signal}\n`)
  },
...
const stopMemoryMonitor = startMemoryMonitor({
  onCritical: (snap, dump) => {
    ...
    process.stderr.write(
      `hades-tui lifecycle: memory critical exit heap=${formatBytes(snap.heapUsed)} rss=${formatBytes(snap.rss)}\n`
    )
    process.stderr.write(dumpNotice(snap, dump))
    process.stderr.write('hades-tui: exiting to avoid OOM; restart to recover\n')
    process.exit(137)
  },
  ...
  onWarn: snap => {
    ...
    process.stderr.write(
      `hades-tui: heap climbing fast (${formatBytes(snap.heapUsed)}) — a large tool output or long session may be straining memory\n`
    )
  }
})
