import { Box, Text, useStdout } from '@hades/ink'
import { useEffect, useState } from 'react'
import unicodeSpinners from 'unicode-animations'

import { artWidth, HADES_SYMBOL_WIDTH, hadesSymbol, logo, LOGO_WIDTH } from '../banner.js'
...
const TAG_FULL = 'Hades Agent · Agent of the Underworld'
const TAG_MID = 'Agent of the Underworld'
const TAG_TINY = 'Hades Agent'
const HIDE_BELOW = 34
...
export function SessionPanel({ info, maxWidth, sid, t }: SessionPanelProps) {
  ...
  const heroLines = hadesSymbol(t.color, t.bannerHero || undefined)
  const leftW = Math.min((artWidth(heroLines) || HADES_SYMBOL_WIDTH) + 4, Math.floor(cols * 0.4))
      ...
      {wide && (
        <Box flexDirection="column" marginRight={2} width={leftW}>
          ...
          <Text color={t.color.accent}>
            ...
            <Text color={t.color.muted}> · Hades Agent</Text>
          </Text>
      ...
      <Box flexDirection="column" width={w}>
        ...
        {typeof info.update_behind === 'number' && info.update_behind > 0 && (
          <Text bold color={t.color.warn}>
            ...
            <Text bold color={t.color.warn}>
              {info.update_command || 'hades update'}
            </Text>
            <Text bold={false} color={t.color.warn} dimColor>
              {' '}
              to update
