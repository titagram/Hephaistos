import { Box, Text, useInput, useStdout } from '@hades/ink'
...
export function ModelPicker({ allowPersistGlobal = true, gw, onCancel, onSelect, sessionId, t }: ModelPickerProps) {
  ...
  useInput((ch, key) => {
                ...
                prev.map(p =>
                  p.slug === provider.slug
                    ? {
                        ...
                        models: [],
                        total_models: 0,
                        warning: p.key_env ? `paste ${p.key_env} to activate` : 'run `hades model` to configure'
                      }
                    : p
                )
              )
    ...
    if (key.return) {
      if (stage === 'provider') {
        ...
        if (provider.authenticated === false) {
          ...
          // Other auth types: no-op (warning shown tells them to run hades model)
          return
        }
    ...
    // filter. With Ctrl held, @hades/ink reports `ch` as the key name ('g'),
