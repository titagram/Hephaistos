...
A command-line interface for Hades Agent, inspired by Claude Code.
...
# ASCII Art - Hades/Pluto symbol (compact, fits in left panel)
HADES_SYMBOL = """[#7A4E2D]⠀⠀⠀⠀⠀⠀⠀⠀⣠⣶⣿⣿⣿⣿⣿⣶⣄⠀⠀⠀⠀⠀⠀⠀⠀[/]
[#A06A3B]⠀⠀⠀⠀⠀⠀⣠⣿⠟⠋⠀⠀⠀⠀⠙⠻⣿⣄⠀⠀⠀⠀⠀⠀[/]
[#B08D57]⠀⠀⠀⠀⠀⣾⡿⠁⠀⠀⠀⠀⠀⠀⠀⠀⠈⢿⣷⠀⠀⠀⠀⠀[/]
[#D7D0C8]⠀⠀⠀⠀⢸⣿⠁⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠈⣿⡇⠀⠀⠀⠀[/]
...
def _build_compact_banner() -> str:
    ...
    if skin_name == "default":
        line1 = "♇ Hades Agent - AI Agent Framework"
        tiny_line = "♇ Hades Agent"
    else:
    ...
    if w < 30:
        return f"\n[{title_color}]{tiny_line}[/] [dim {dim_color}]- Hades Agent[/]\n"
...
class HermesCLI(CLIAgentSetupMixin, CLICommandsMixin):
    """
    Interactive CLI for Hades Agent.
    ...
    def show_banner(self):
        ...
        if is_nous_hermes_non_agentic(model_name):
            self._console_print()
            self._console_print(
                "[bold yellow]⚠  Nous-hosted Hermes 3 & 4 models are NOT agentic and are not "
    ...
    def _handle_model_switch(self, cmd_original: str):
        ...
        Supports:
          ...
          /model <name> --session             — switch for this session only (compatibility)
          /model <name> --global              — switch and persist (explicit)
          /model <name> --provider <provider> — switch provider + model
          /model --provider <provider>        — switch to provider, auto-detect model

        A model switch is session-only unless ``--global`` is explicitly set.
        ``--session`` remains accepted for compatibility.
        """
        ...
        # Resolve the effective persistence once: only an unambiguous
        # ``--global`` request persists shared configuration.
    ...
    def process_command(self, command: str) -> bool:
        ...
        elif canonical == "gnothi_seauton":
            self._handle_gnothi_seauton_command(cmd_original)
    ...
    def run(self):
        ...
        def handle_enter(event):
            ...
            if self._model_picker_state:
                try:
                    # Picker selections are session-only, like /model <name>.
