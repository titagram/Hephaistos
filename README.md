...
The installer handles everything: uv, Python 3.11, Node.js, ripgrep, ffmpeg, **and a portable Git Bash** (MinGit, unpacked to `%LOCALAPPDATA%\hades\git` — no admin required, completely isolated from any system Git install). Hades uses this bundled Git Bash to run shell commands.
...
>
> **Windows:** Native Windows is fully supported — the PowerShell one-liner above installs everything. If you'd rather use WSL2, the Linux command works there too. Native Windows install lives under `%LOCALAPPDATA%\hades`; WSL2 installs under `~/.hades` as on Linux.
...
If your antivirus (Bitdefender, Windows Defender, etc.) quarantines `uv.exe` from the Hades `bin` folder (`%LOCALAPPDATA%\hades\bin\uv.exe`), this is a **false positive**. The file is Astral's `uv` — the Rust Python package manager Hades bundles to manage its Python environment. ML-based antivirus engines commonly flag unsigned Rust binaries that download and install packages.
...
# Run verification
$uv = "$env:LOCALAPPDATA\hades\bin\uv.exe"
...
**To whitelist Hades:**
- **Windows Defender:** Run PowerShell as Admin → `Add-MpPreference -ExclusionPath "$env:LOCALAPPDATA\hades\bin"`
...
- **Skills** — user-created skills → `~/.hades/skills/openclaw-imports/`
...
full git checkout it creates at `$HADES_HOME/hades-agent` (usually
`~/.hades/hades-agent`). This matches the layout used by `hades update`, the
managed venv, lazy dependencies, gateway, and docs tooling.

```bash
curl -fsSL https://hades-agent.local/install.sh | bash
cd "${HADES_HOME:-$HOME/.hades}/hades-agent"
