...
""",
    ...
    for name in ("marker_route", "alias_route", "multiple_route"):
        dependencies = [
            item
            for item in adapter.pipeline(context, by_name[name])
            if item.framework_role == "decorator_dependency"
...
def test_middleware_invoked_local_helpers_bound_captured_storage_mutation(
    tmp_path: Path,
) -> None:
    _prepare(tmp_path)
    _write(
...
@exact_app.middleware("http")
async def dormant_helper(request, proceed):
    callbacks = [proceed]
    def overwrite():
        callbacks[0] = replacement
    return await callbacks[0](request)
...
@mutated_app.middleware("http")
async def invoked_helper(request, proceed):
    callbacks = [proceed]
    def overwrite():
        callbacks[0] = replacement
    overwrite()
...
@mutated_app.middleware("http")
async def transitive_helper(request, proceed):
    callbacks = [proceed]
    def overwrite():
        callbacks[0] = replacement
    def invoke():
...
@mutated_app.middleware("http")
async def nonlocal_helper(request, proceed):
    next_step = proceed
    def overwrite():
        nonlocal next_step
        next_step = replacement
    overwrite()
    return await next_step(request)

@mutated_app.middleware("http")
async def method_helper(request, proceed):
    callbacks = [proceed]
    def mutate():
        callbacks.clear()
    mutate()
...
def test_wildcard_exports_apply_only_proven_all_method_mutations(
    tmp_path: Path,
) -> None:
    _prepare(tmp_path)
    _write(
...
__all__ = ["appended_marker"]
__all__.append("extended_alias")
__all__.extend(("augmented_marker",))
__all__ += ["FiniteError"]

def shared(): return None
appended_marker = Depends(shared)
extended_alias = Annotated[str, Depends(shared)]
augmented_marker = Depends(shared)
class FiniteError(Exception): pass
""",
    )
    _write(
        tmp_path,
...
__all__ = ["dynamic_marker"]
__all__.append(build_export())
def shared(): return None
dynamic_marker = Depends(shared)
""",
    )
    _write(
        tmp_path,
...
__all__ = ["conditional_marker"]
if enabled:
    __all__.extend(("other",))
def shared(): return None
conditional_marker = Depends(shared)
""",
    )
    _write(
        tmp_path,
...
@app.get("/appended")
async def appended_route(value=appended_marker): return None

@app.get("/extended")
async def extended_route(value: extended_alias): return None

@app.get("/augmented")
async def augmented_route(value=augmented_marker): return None

@app.exception_handler(FiniteError)
async def finite_handler(request, exc): return None

@app.get("/error")
async def error_route(): raise FiniteError()
""",
    )
    _write(
        tmp_path,
...
@app.get("/conditional")
async def conditional_route(value=conditional_marker): return None
""",
    )
    paths = (
        "app.py",
    ...
    for name in ("appended_route", "extended_route", "augmented_route"):
        dependencies = [
            item
            for item in adapter.pipeline(context, by_name[name])
            if item.framework_role == "decorator_dependency"
    ...
    assert "exception_handler" in {
        item.framework_role
        for item in adapter.pipeline(context, by_name["error_route"])
    }
    coverage = adapter.coverage_events(context)
    for path in ("dynamic_app.py", "conditional_app.py"):
        assert any(
            event.path == path
            and event.reason_code == "framework_config_unresolved"
            and event.outcome is CoverageOutcome.PARTIAL
...
def test_relative_from_import_tracks_loaded_submodule_reachability(
    tmp_path: Path,
) -> None:
    _prepare(tmp_path)
...
@app.get("/safe")
async def safe_route(value=providers.marker): return None
""",
    )
    _write(
        tmp_path,
        "pkg/cycle_app.py",
        """from . import cycle_routes
from fastapi import Depends
...
""",
    ...
    _write(
        tmp_path,
        "pkg/cycle_routes.py",
        """from . import cycle_app as partial
from fastapi import FastAPI

app = FastAPI()

@app.get("/cycle")
async def cycle_route(value=partial.marker): return None
""",
    )
    paths = (
        "pkg/__init__.py",
    ...
    assert isinstance(dependency.target, FrameworkLocalTarget)
    assert dependency.target.local_key == _function_key_at(
        tmp_path,
        "pkg/providers.py",
        "shared",
    )
    assert any(
        event.path == "pkg/cycle_routes.py"
        and event.reason_code == "framework_config_unresolved"
        and event.outcome is CoverageOutcome.PARTIAL
        for event in adapter.coverage_events(context)
    )
...
def test_dependency_destructuring_allows_one_star_per_nested_sequence(
    tmp_path: Path,
) -> None:
    _prepare(tmp_path)
    _write(
...
(marker, *outer, (DependencyAlias, *inner)) = (
    Depends(shared),
    1,
    2,
    (Annotated[str, Depends(shared)], 3, 4),
)

dynamic = Depends(shared)
(dynamic, *dynamic_outer, (DynamicAlias, *dynamic_inner)) = (
    Depends(shared),
    build_nested(),
)

incompatible = Depends(shared)
(incompatible, *short_outer, (short_last, *short_inner)) = (Depends(shared),)
...
@app.get("/outer")
async def outer_route(value=outer): return None

@app.get("/inner")
async def inner_route(value=inner): return None
...
""",
    ...
    for name in ("marker_route", "alias_route"):
        ...
        assert isinstance(dependencies[0].target, FrameworkLocalTarget)
        assert dependencies[0].target.local_key == _function_key(tmp_path, "shared")
    for name in ("outer_route", "inner_route"):
        assert not any(
            item.framework_role == "decorator_dependency"
            for item in adapter.pipeline(context, by_name[name])
        )
...
def test_middleware_literal_container_mutations_are_key_and_path_sensitive(
    tmp_path: Path,
) -> None:
    _prepare(tmp_path)
    _write(
...
@app.middleware("http")
async def unrelated_nested(request, proceed):
    callbacks = {"next": proceed, "other": [replacement]}
    callbacks["other"][0] = another
    return await callbacks["next"](request)

@app.middleware("http")
async def helper_unrelated(request, proceed):
    callbacks = {"next": proceed, "other": replacement}
    def mutate():
        callbacks["other"] = another
    mutate()
    return await callbacks["next"](request)
...
@app.middleware("http")
async def related(request, proceed):
    callbacks = {"next": proceed}
    callbacks["next"] = replacement
    return await callbacks["next"](request)

@app.get("/related")
async def related_endpoint(): return None
""",
    )
    _write(
        tmp_path,
...
@app.middleware("http")
async def equal_key(request, proceed):
    callbacks = {1: proceed}
    callbacks[True] = replacement
    return await callbacks[1.0](request)

@app.get("/equal")
async def equal_endpoint(): return None
""",
    )
    _write(
        tmp_path,
...
@app.middleware("http")
async def unknown_key(request, proceed):
    callbacks = {"next": proceed}
    callbacks[choose_key()] = replacement
    return await callbacks["next"](request)

@app.get("/unknown")
async def unknown_endpoint(): return None
""",
    )
    paths = ("equal.py", "exact.py", "related.py", "unknown.py")
    adapter = FastAPILifecycleAdapter()
    context = _context(tmp_path)
    entrypoints = adapter.entrypoints(
        context,
    ...
    for name in ("unrelated_nested", "helper_unrelated"):
        assert not middleware(
            "exact_endpoint",
            "exact.py",
            name,
        ).short_circuit_successors
    for route_name, path, name in (
        ("related_endpoint", "related.py", "related"),
        ("equal_endpoint", "equal.py", "equal_key"),
        ("unknown_endpoint", "unknown.py", "unknown_key"),
    ):
        assert middleware(route_name, path, name).short_circuit_successors

    coverage = adapter.coverage_events(context)
    for path in ("exact.py", "related.py", "equal.py"):
        assert not any(
            event.path == path and event.reason_code == "middleware_behavior_unresolved"
            for event in coverage
        )
    assert any(
        event.path == "unknown.py"
...
def test_middleware_local_helper_alias_calls_are_occurrence_bounded(
    tmp_path: Path,
) -> None:
    _prepare(tmp_path)
    _write(
...
@app.middleware("http")
async def dormant_alias(request, proceed):
    callbacks = [proceed]
    def overwrite():
        callbacks[0] = replacement
    helper = overwrite
...
@app.get("/dormant")
async def dormant_endpoint(): return None
""",
    )
    _write(
        tmp_path,
...
@app.middleware("http")
async def invoked_alias(request, proceed):
    callbacks = [proceed]
    def overwrite():
        callbacks[0] = replacement
    helper = overwrite
...
@app.get("/invoked")
async def invoked_endpoint(): return None
""",
    )
    _write(
        tmp_path,
...
@app.middleware("http")
async def dynamic_alias(request, proceed):
    callbacks = [proceed]
    def overwrite():
        callbacks[0] = replacement
    helper = choose(overwrite, replacement)
    helper()
    return await callbacks[0](request)

@app.get("/dynamic")
async def dynamic_endpoint(): return None
""",
    )
    paths = ("dormant.py", "dynamic.py", "invoked.py")
    adapter = FastAPILifecycleAdapter()
    context = _context(tmp_path)
    entrypoints = adapter.entrypoints(
        context,
    ...
    for route_name, path, name in (
        ("invoked_endpoint", "invoked.py", "invoked_alias"),
        ("dynamic_endpoint", "dynamic.py", "dynamic_alias"),
    ):
        assert middleware(route_name, path, name).short_circuit_successors

    coverage = adapter.coverage_events(context)
    assert not any(
        event.path == "dormant.py"
        and event.reason_code == "middleware_behavior_unresolved"
        for event in coverage
    )
    for path in ("dynamic.py", "invoked.py"):
        assert any(
            event.path == path
            and event.reason_code == "middleware_behavior_unresolved"
            and event.outcome is CoverageOutcome.PARTIAL
...
def test_middleware_container_aliases_share_keyed_mutation_identity(
    tmp_path: Path,
) -> None:
    _prepare(tmp_path)
    _write(
...
@app.middleware("http")
async def direct_alias(request, proceed):
    callbacks = {"next": proceed}
    alias = callbacks
    alias["next"] = replacement
    return await callbacks["next"](request)

@app.middleware("http")
async def nested_alias(request, proceed):
    callbacks = {"inner": {"next": proceed}}
    outer_alias = callbacks
    inner_alias = outer_alias["inner"]
    inner_alias["next"] = replacement
    return await callbacks["inner"]["next"](request)

@app.middleware("http")
async def read_only_alias(request, proceed):
    callbacks = {"inner": {"next": proceed}}
    outer_alias = callbacks
    inner_alias = outer_alias["inner"]
    return await inner_alias["next"](request)
...
@app.middleware("http")
async def dynamic_alias_key(request, proceed):
    callbacks = {"next": proceed}
    alias = callbacks
    alias[choose_key()] = replacement
    return await callbacks["next"](request)
...
""",
    ...
    for name in ("direct_alias", "nested_alias"):
        assert middleware(
            "exact_endpoint",
            "exact.py",
            name,
...
def test_wildcard_exports_preserve_all_alias_mutation_identity(
    tmp_path: Path,
) -> None:
    _prepare(tmp_path)
    _write(
...
__all__ = ["appended_marker"]
exports = __all__
exports.append("extended_alias")
more_exports = exports
more_exports.extend(("augmented_marker",))
more_exports += ["FiniteError"]
...
""",
    ...
    paths = ("app.py", "dynamic.py", "dynamic_app.py", "finite.py")
    adapter = FastAPILifecycleAdapter()
    context = _context(tmp_path)
    entrypoints = adapter.entrypoints(
        context,
...
def test_dependency_destructuring_loads_rhs_before_simultaneous_stores(
    tmp_path: Path,
) -> None:
    _prepare(tmp_path)
    _write(
...
marker, old_marker = Depends(new_dependency), marker

app = FastAPI()

@app.get("/new")
async def new_route(value=marker): return None

@app.get("/old")
async def old_route(value=old_marker): return None
""",
    )
    adapter = FastAPILifecycleAdapter()
    context = _context(tmp_path)
    ...
    by_name = {item.public_name: item for item in entrypoints}
    assert set(by_name) == {"new_route", "old_route"}
    for name, target_name in (
        ("new_route", "new_dependency"),
        ("old_route", "old_dependency"),
    ):
        dependencies = [
            item
            for item in adapter.pipeline(context, by_name[name])
        ...
        assert len(dependencies) == 1
        assert isinstance(dependencies[0].target, FrameworkLocalTarget)
        assert dependencies[0].target.local_key == _function_key(
            tmp_path,
            target_name,
        )
    assert not any(
...
def test_middleware_destructured_helper_alias_calls_are_occurrence_bounded(
    tmp_path: Path,
) -> None:
    _prepare(tmp_path)
    _write(
...
async def dynamic_alias(request, proceed):
    ...
    def overwrite():
        callbacks[0] = replacement
    (helper,) = choose_helpers(overwrite)
    helper()
    return await callbacks[0](request)
...
def test_middleware_same_name_helpers_resolve_the_body_at_each_call(
    tmp_path: Path,
) -> None:
    _prepare(tmp_path)
    _write(
...
@app.middleware("http")
async def first_effect(request, proceed):
    callbacks = [proceed]
    def mutate():
        callbacks[0] = replacement
    mutate()
...
@app.get("/first")
async def first_endpoint(): return None
""",
    )
    _write(
        tmp_path,
...
@app.middleware("http")
async def late_effect(request, proceed):
    callbacks = [proceed]
    def mutate():
        return None
    mutate()
...
@app.get("/late")
async def late_endpoint(): return None
""",
    )
    adapter = FastAPILifecycleAdapter()
    context = _context(tmp_path)
    entrypoints = adapter.entrypoints(
        context,
        (
            _syntax(tmp_path, "first_effect.py"),
            _syntax(tmp_path, "late_effect.py"),
        ),
    )
    ...
    coverage = adapter.coverage_events(context)
    assert any(
        event.path == "first_effect.py"
        and event.reason_code == "middleware_behavior_unresolved"
        and event.outcome is CoverageOutcome.PARTIAL
        for event in coverage
    )
    assert not any(
        event.path == "late_effect.py"
        and event.reason_code == "middleware_behavior_unresolved"
        for event in coverage
    )


def test_middleware_chained_assignment_shares_one_container_identity(
    tmp_path: Path,
) -> None:
    _prepare(tmp_path)
    _write(
...
@app.middleware("http")
async def chained_mutation(request, proceed):
    alias = callbacks = {"next": proceed}
    alias["next"] = replacement
    return await callbacks["next"](request)

@app.middleware("http")
async def chained_read_only(request, proceed):
    alias = callbacks = {"next": proceed}
    return await alias["next"](request)

@app.middleware("http")
async def independent_storage(request, proceed):
    callbacks = {"next": proceed}
    alias = {"next": replacement}
    alias["next"] = another
    return await callbacks["next"](request)
...
""",
    ...
    assert middleware("chained_mutation").short_circuit_successors
    for name in ("chained_read_only", "independent_storage"):
        assert not middleware(name).short_circuit_successors
    assert not any(
        event.path == "app.py" and event.reason_code == "middleware_behavior_unresolved"
        for event in adapter.coverage_events(context)
    )


def test_wildcard_exports_track_destructured_all_alias_mutations(
    tmp_path: Path,
) -> None:
    _prepare(tmp_path)
    _write(
...
__all__ = ["marker"]
(exports,) = (__all__,)
exports.append("alias_marker")

def shared(): return None
marker = Depends(shared)
alias_marker = Depends(shared)
""",
    )
    _write(
        tmp_path,
...
@app.get("/alias")
async def alias_route(value=alias_marker): return None
""",
    )
    _write(
        tmp_path,
...
""",
    ...
    by_name = {item.public_name: item for item in entrypoints}
    assert set(by_name) == {"marker_route", "alias_route"}
    for route in by_name.values():
        dependencies = [
            item
            for item in adapter.pipeline(context, route)
            if item.framework_role == "decorator_dependency"
...
def test_dependency_destructuring_applies_namedexpr_effects_left_to_right(
    tmp_path: Path,
) -> None:
    _prepare(tmp_path)
    _write(
...
marker = Depends(old_dependency)
assigned, later = ((marker := Depends(new_dependency)), marker)

app = FastAPI()

@app.get("/assigned")
async def assigned_route(value=assigned): return None

@app.get("/later")
async def later_route(value=later): return None
...
marker = Depends(old_dependency)
assigned, later = ((marker := build_marker()), marker)

app = FastAPI()

@app.get("/assigned")
async def dynamic_assigned_route(value=assigned): return None

@app.get("/later")
async def dynamic_later_route(value=later): return None
...
""",
    ...
    by_name = {item.public_name: item for item in entrypoints}
    assert set(by_name) == {"assigned_route", "later_route", "marker_route"}
    for route in by_name.values():
        dependencies = [
            item
            for item in adapter.pipeline(context, route)
...
def test_middleware_invoked_helper_applies_nonlocal_callable_rebinding(
    tmp_path: Path,
) -> None:
    _prepare(tmp_path)
    _write(
...
@app.middleware("http")
async def exact_rebind(request, proceed):
    callbacks = [proceed]
    def noop():
        return None
    def overwrite():
...
@app.middleware("http")
async def dynamic_rebind(request, proceed):
    callbacks = [proceed]
    def noop():
        return None
    def overwrite():
    ...
    def rebind():
        nonlocal helper
        helper = choose(overwrite)
    rebind()
    helper()
    return await callbacks[0](request)

@app.middleware("http")
async def dormant_rebind(request, proceed):
    callbacks = [proceed]
    def noop():
        return None
    def overwrite():
...
@app.middleware("http")
async def rebind_after_call(request, proceed):
    callbacks = [proceed]
    def noop():
        return None
    def overwrite():
...
""",
    ...
    for name in ("exact_rebind", "dynamic_rebind"):
        assert middleware(name).short_circuit_successors
    for name in ("dormant_rebind", "rebind_after_call"):
        assert not middleware(name).short_circuit_successors
...
def test_middleware_local_def_defaults_apply_namedexpr_helper_effects(
    tmp_path: Path,
) -> None:
    _prepare(tmp_path)
    _write(
...
@app.middleware("http")
async def exact_default(request, proceed):
    callbacks = [proceed]
    def noop():
        return None
    def overwrite():
        callbacks[0] = replacement
    helper = noop
    def configured(value=(helper := overwrite)):
        return value
    helper()
    return await callbacks[0](request)

@app.middleware("http")
async def dynamic_default(request, proceed):
    callbacks = [proceed]
    def noop():
        return None
    def overwrite():
        callbacks[0] = replacement
    helper = noop
    def configured(value=(helper := choose(overwrite))):
        return value
    helper()
    return await callbacks[0](request)

@app.middleware("http")
async def dormant_body(request, proceed):
    callbacks = [proceed]
    def noop():
        return None
    def overwrite():
...
""",
    ...
    for name in ("exact_default", "dynamic_default"):
        assert middleware(name).short_circuit_successors
    assert not middleware("dormant_body").short_circuit_successors
...
def test_middleware_finite_starred_helper_destructuring_is_exact(
    tmp_path: Path,
) -> None:
    _prepare(tmp_path)
    _write(
...
@app.middleware("http")
async def finite_safe_star(request, proceed):
    callbacks = [proceed]
    def noop():
        return None
    first, *helpers = (noop, noop)
    helpers[0]()
    return await callbacks[0](request)

@app.get("/finite")
async def finite_endpoint(): return None
""",
    )
    _write(
        tmp_path,
...
""",
    ...
    entrypoints = adapter.entrypoints(
        context,
        (_syntax(tmp_path, "dynamic.py"), _syntax(tmp_path, "finite.py")),
    )
    by_name = {item.public_name: item for item in entrypoints}
...
def test_wildcard_exports_distinguish_namedexpr_rebind_from_alias_escape(
    tmp_path: Path,
) -> None:
    _prepare(tmp_path)
    _write(
...
__all__ = ["rebound_marker"]
exports = __all__
opaque({"nested": (exports := ["private_name"])})

def rebound_dependency(): return None
rebound_marker = Depends(rebound_dependency)
""",
    )
    _write(
        tmp_path,
...
@app.get("/rebound")
async def rebound_route(value=rebound_marker): return None
""",
    )
    _write(
        tmp_path,
...
""",
    ...
    paths = ("escaped.py", "escaped_app.py", "rebound.py", "rebound_app.py")
    adapter = FastAPILifecycleAdapter()
    context = _context(tmp_path)
    entrypoints = adapter.entrypoints(
        context,
    ...
    assert {item.public_name for item in entrypoints} == {"rebound_route"}
    dependencies = [
        item
        for item in adapter.pipeline(context, entrypoints[0])
        if item.framework_role == "decorator_dependency"
    ]
    assert len(dependencies) == 1
    assert isinstance(dependencies[0].target, FrameworkLocalTarget)
...
def test_wildcard_exports_keep_finite_public_starred_destructuring_exact(
    tmp_path: Path,
) -> None:
    _prepare(tmp_path)
    _write(
...
def finite_dependency(): return None
finite_marker, *finite_tail = (Depends(finite_dependency), 1, 2)
""",
    )
    _write(
        tmp_path,
...
def test_dependency_function_defaults_apply_namedexpr_effects_to_timeline(
    tmp_path: Path,
) -> None:
    _prepare(tmp_path)
    _write(
...
def configured(value=(marker := Depends(new_dependency))):
    return value

@app.get("/after")
async def after_route(value=marker): return None
""",
    )
    _write(
        tmp_path,
...
def configured(value=(marker := build_marker())):
    return value

@app.get("/after")
async def dynamic_after_route(value=marker): return None
""",
    )
    adapter = FastAPILifecycleAdapter()
    context = _context(tmp_path)
    ...
    expected = {
        "after_route": ("exact.py", "new_dependency"),
        "before_route": ("exact.py", "old_dependency"),
        "dynamic_before_route": ("dynamic.py", "old_dependency"),
    }
    for route_name, (path, dependency_name) in expected.items():
...
def test_middleware_invoked_helper_replay_preserves_execution_order(
    tmp_path: Path,
) -> None:
    _prepare(tmp_path)
    _write(
...
@app.middleware("http")
async def before_rebind(request, proceed):
    callbacks = [proceed]
    def noop():
        return None
    def overwrite():
...
@app.get("/before")
async def before_endpoint(): return None
""",
    )
    _write(
        tmp_path,
...
@app.middleware("http")
async def after_rebind(request, proceed):
    callbacks = [proceed]
    def noop():
        return None
    def overwrite():
...
@app.get("/after")
async def after_endpoint(): return None
""",
    )
    _write(
        tmp_path,
...
@app.middleware("http")
async def unreachable_rebind(request, proceed):
    callbacks = [proceed]
    def noop():
        return None
    def overwrite():
...
@app.get("/unreachable")
async def unreachable_endpoint(): return None
""",
    )
    paths = ("after.py", "before.py", "unreachable.py")
    adapter = FastAPILifecycleAdapter()
    context = _context(tmp_path)
    entrypoints = adapter.entrypoints(
        context,
    ...
    coverage = adapter.coverage_events(context)
    for path in ("before.py", "unreachable.py"):
        assert not any(
            event.path == path and event.reason_code == "middleware_behavior_unresolved"
            for event in coverage
        )
...
def test_middleware_local_function_defaults_execute_helper_calls_eagerly(
    tmp_path: Path,
) -> None:
    _prepare(tmp_path)
    _write(
...
@app.middleware("http")
async def eager_effect(request, proceed):
    callbacks = [proceed]
    def overwrite():
        callbacks[0] = replacement
    def configured(value=overwrite()):
        return value
    return await callbacks[0](request)

@app.get("/effect")
async def effect_endpoint(): return None
""",
    )
    _write(
        tmp_path,
...
@app.middleware("http")
async def eager_safe(request, proceed):
    callbacks = [proceed]
    def noop():
        return None
    def configured(value=noop()):
        return value
    return await callbacks[0](request)

@app.get("/safe")
async def safe_endpoint(): return None
""",
    )
    _write(
        tmp_path,
...
@app.middleware("http")
async def lazy_body(request, proceed):
    callbacks = [proceed]
    def overwrite():
        callbacks[0] = replacement
    def configured():
...
@app.get("/lazy")
async def lazy_endpoint(): return None
""",
    )
    _write(
        tmp_path,
...
@app.middleware("http")
async def eager_lambda_default(request, proceed):
    callbacks = [proceed]
    def overwrite():
        callbacks[0] = replacement
    def configured(callback=lambda value=overwrite(): value):
        return callback
    return await callbacks[0](request)

@app.get("/lambda-default")
async def lambda_default_endpoint(): return None
""",
    )
    paths = ("effect.py", "lambda_default.py", "lazy.py", "safe.py")
    adapter = FastAPILifecycleAdapter()
    context = _context(tmp_path)
    entrypoints = adapter.entrypoints(
        context,
    ...
    assert middleware(
        "lambda_default_endpoint",
        "lambda_default.py",
        "eager_lambda_default",
    ).short_circuit_successors
    for route_name, path, middleware_name in (
        ("safe_endpoint", "safe.py", "eager_safe"),
        ("lazy_endpoint", "lazy.py", "lazy_body"),
    ):
        assert not middleware(
            route_name, path, middleware_name
        ).short_circuit_successors

    coverage = adapter.coverage_events(context)
    for path in ("effect.py", "lambda_default.py"):
        assert any(
            event.path == path
            and event.reason_code == "middleware_behavior_unresolved"
            and event.outcome is CoverageOutcome.PARTIAL
...
def test_wildcard_exports_apply_namedexpr_rebinding_in_expression_order(
    tmp_path: Path,
) -> None:
    _prepare(tmp_path)
    _write(
...
__all__ = ["rebound_marker"]
exports = __all__
opaque((exports := ["private_name"]), exports)
...
__all__ = ["escaped_marker"]
exports = __all__
opaque(exports, (exports := ["private_name"]))
...
""",
    ...
    _write(
        tmp_path,
        "subscript_rebound.py",
        """from fastapi import Depends

__all__ = ["subscript_marker"]
exports = __all__
opaque((exports := [None])[0], exports)

def subscript_dependency(): return None
subscript_marker = Depends(subscript_dependency)
""",
    )
    _write(
        tmp_path,
        "subscript_app.py",
        """from subscript_rebound import *
from fastapi import FastAPI

app = FastAPI()

@app.get("/subscript")
async def subscript_route(value=subscript_marker): return None
""",
    )
    paths = (
        "escaped_app.py",
    ...
    by_name = {item.public_name: item for item in entrypoints}
    assert set(by_name) == {"rebound_route", "subscript_route"}
    for route_name, path, dependency_name in (
        ("rebound_route", "rebound_first.py", "rebound_dependency"),
        (
            "subscript_route",
            "subscript_rebound.py",
            "subscript_dependency",
        ),
    ):
        dependencies = [
            item
    ...
    coverage = adapter.coverage_events(context)
    assert not any(
        event.path in {"rebound_app.py", "subscript_app.py"}
        and event.reason_code == "framework_config_unresolved"
        for event in coverage
    )
    assert any(
...
def test_dependency_function_decorators_and_annotations_apply_eager_effects(
    tmp_path: Path,
) -> None:
    _prepare(tmp_path)
    _write(
...
@app.get("/annotation")
async def annotation_route(value=marker): return None
""",
    )
    _write(
        tmp_path,
...
def old_dependency(): return None
def passthrough(function): return function
...
@app.get("/decorator")
async def decorator_route(value=marker): return None
""",
    )
    _write(
        tmp_path,
...
@app.get("/dynamic")
async def dynamic_route(value=marker): return None
""",
    )
    _write(
        tmp_path,
...
@app.get("/postponed")
async def postponed_route(value=marker): return None
""",
    )
    _write(
        tmp_path,
...
def old_dependency(): return None
def vararg_dependency(): return None
def keyword_dependency(): return None

marker = Depends(old_dependency)

def configured(
    *items: (marker := Depends(vararg_dependency)),
    keyword: (marker := Depends(keyword_dependency)),
):
...
@app.get("/ordered")
async def ordered_route(value=marker): return None
""",
    )
    paths = (
        "annotation.py",
    ...
    expected = {
        "annotation_route": ("annotation.py", "new_dependency"),
        "ordered_route": ("ordered.py", "keyword_dependency"),
        "postponed_route": ("postponed.py", "old_dependency"),
    }
    for route_name, (path, dependency_name) in expected.items():
        dependencies = [
            item
    ...
    decorator_dependencies = [
        item
        for item in adapter.pipeline(context, by_name["decorator_route"])
        if item.framework_role == "decorator_dependency"
    ]
    assert decorator_dependencies == []

    coverage = adapter.coverage_events(context)
    for path in ("annotation.py", "decorator.py", "ordered.py", "postponed.py"):
        assert not any(
            event.path == path and event.reason_code == "framework_config_unresolved"
            for event in coverage
        )
    assert any(
        event.path == "dynamic.py"
...
def test_wildcard_exports_preserve_nested_all_identity_in_starred_capture(
    tmp_path: Path,
) -> None:
    _prepare(tmp_path)
    _write(
...
__all__ = ["marker"]
*containers, tail = (__all__, "tail")
containers[0].append("alias_marker")

def shared_dependency(): return None
marker = Depends(shared_dependency)
alias_marker = Depends(shared_dependency)
""",
    )
    _write(
        tmp_path,
...
__all__ = ["safe_marker"]
*containers, tail = ([], "tail")
containers[0].append("private_name")

def safe_dependency(): return None
safe_marker = Depends(safe_dependency)
private_name = Depends(safe_dependency)
""",
    )
    _write(
        tmp_path,
...
@app.get("/private")
async def private_route(value=private_name): return None
""",
    )
    _write(
        tmp_path,
...
@app.get("/uncertain")
async def uncertain_route(value=alias_marker): return None
""",
    )
    paths = (
        "escaped.py",
    ...
    for route_name in ("alias_route", "marker_route", "safe_route"):
        dependencies = [
            item
            for item in adapter.pipeline(context, by_name[route_name])
            if item.framework_role == "decorator_dependency"
    ...
    private_dependencies = [
        item
        for item in adapter.pipeline(context, by_name["private_route"])
        if item.framework_role == "decorator_dependency"
    ]
    assert private_dependencies == []

    coverage = adapter.coverage_events(context)
    for path in ("nested_app.py", "safe_app.py"):
        assert not any(
            event.path == path and event.reason_code == "framework_config_unresolved"
            for event in coverage
        )
    assert any(
        event.path == "uncertain_app.py"
        and event.reason_code == "framework_config_unresolved"
        and event.outcome is CoverageOutcome.PARTIAL
        for event in coverage
    )
...
def test_dependency_lambda_bodies_do_not_apply_default_namedexpr_effects(
    tmp_path: Path,
) -> None:
    _prepare(tmp_path)
    _write(
...
def configured(callback=lambda: (marker := Depends(new_dependency))):
    return callback

app = FastAPI()

@app.get("/lazy")
async def lazy_route(value=marker): return None
""",
    )
    _write(
        tmp_path,
...
def configured(callback=lambda: (marker := choose_dependency())):
    return callback
...
def configured(
    callback=lambda value=(marker := Depends(new_dependency)): value,
):
    return callback
...
@app.get("/eager")
async def eager_route(value=marker): return None
""",
    )
    paths = ("dynamic.py", "eager.py", "lazy.py")
    adapter = FastAPILifecycleAdapter()
    context = _context(tmp_path)
    entrypoints = adapter.entrypoints(
        context,
    ...
    by_name = {item.public_name: item for item in entrypoints}
    assert set(by_name) == {"dynamic_route", "eager_route", "lazy_route"}
    expected = {
        "dynamic_route": ("dynamic.py", "old_dependency"),
        "eager_route": ("eager.py", "new_dependency"),
        "lazy_route": ("lazy.py", "old_dependency"),
    }
    for route_name, (path, dependency_name) in expected.items():
        dependencies = [
            item
