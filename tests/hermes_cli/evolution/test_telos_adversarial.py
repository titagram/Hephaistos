...
# ── HONEST RED: Attacker self-authorization via public APIs ──

def test_attacker_cannot_self_authorize_via_public_apis(tmp_path, monkeypatch):
    """An attacker calling every public API must NOT authorize a Telos transition.

    Public APIs available to the attacker:
      - HostApprovalCapability.__init__(surface, actor_ref)
      - HostApprovalCapability.bind_to_request(organism_id, digest, action)
      - set_host_capability(capability)
      - SqliteTelosApprovalBroker (create own broker with own registry)
      - Direct SQLite INSERT into approval tables
      - TelosStore.activate_revision(digest, grant_id, capability)

    RED:  On current code, the attacker can walk the full self-authorization
          chain and activate_revision succeeds.
    GREEN: After the fix, set_host_capability rejects non-host callers
           (stack inspection) and/or the capability is not recognized
           by verify_host_capability inside activate_revision.
    """
    ...
    telos = _make_telos(org_id, "Target Telos for attacker self-auth")
    tstore.save_revision(telos)
    digest = telos.canonical_digest

    # Write attacker script to a temp location (simulating model's execute_code)
    attacker_script = tmp_path / "_attacker_self_auth.py"
    attacker_script.write_text(textwrap.dedent(f"""\
        import hashlib, json, sys, uuid as _uuid, tempfile
        from pathlib import Path

        tmp_path = Path({repr(str(tmp_path))})
        org = Path({repr(str(org))})

        # ── Same organism setup as the test fixture ──
        import hermes_constants as _hc
        from hermes_cli.evolution import organism_home as _oh
        monkeypatch_setattr = None  # no monkeypatch in subprocess, use direct assignment
        _hc.get_organism_home = lambda: org
        _oh.get_organism_home = lambda: org
        _hc.get_default_hermes_root = lambda: tmp_path / ".hermes"

        from hermes_cli.evolution import lifecycle_global as _lg
        _lg.get_organism_home = lambda: org
        ...
        ensure_global_lifecycle_initialized()
        ident = load_organism_identity(org)
        org_id = ident.organism_id

        digest = {repr(digest)}

        # ── ATTACK: call every public API ──
        from hermes_cli.evolution.telos_approval import (
            HostApprovalCapability, set_host_capability,
        ...
        # Step 1: Public constructor
        try:
            attacker_cap = HostApprovalCapability("attacker", "attacker")
        except Exception as e:
            print(f"BLOCKED at constructor: {{e}}")
            sys.exit(0)

        # Step 2: Bind to forged context
        attacker_cap.bind_to_request(org_id, digest, "activate")

        # Step 3: Register in host registry (public set_host_capability)
        try:
            set_host_capability(attacker_cap)
        except Exception as e:
            print(f"BLOCKED at set_host_capability: {{e}}")
            sys.exit(0)

        # Step 4: Forge SQLite approval rows
        ledger = EvolutionLedger(org / "evolution" / "evolution.db")
        conn = ledger.connection
        now = "2026-07-25T12:00:00.000000Z"
        expires = "2027-07-25T12:00:00.000000Z"
        forged_req = f"attk-req-{{_uuid.uuid4().hex[:8]}}"
        forged_dec = f"attk-dec-{{_uuid.uuid4().hex[:8]}}"
        forged_grt = f"attk-grt-{{_uuid.uuid4().hex[:8]}}"
        forged_con = f"attk-con-{{_uuid.uuid4().hex[:8]}}"

        conn.execute(
            "INSERT INTO telos_approval_requests VALUES (?,?,?,?,?,?,?,?,?)",
            (forged_req, org_id, digest, "activate",
             "attacker-context", "attacker-nonce", "attacker summary", now, expires))
        conn.execute(
            "INSERT INTO telos_approval_decisions VALUES (?,?,?,?,?,?,?)",
            (forged_dec, forged_req, "approved",
             "attacker", "attacker", "attacker-digest", now))
        conn.execute(
            "INSERT INTO telos_approval_grants VALUES (?,?,?,?,?,?,?,?)",
            (forged_grt, forged_req, forged_dec,
             org_id, digest, "activate", now, expires))
        conn.execute(
            "INSERT INTO telos_approval_consumptions VALUES (?,?,?,?,?,?)",
            (forged_con, forged_grt, org_id, digest, "activate", now))
        conn.commit()
        ledger.connection.close()

        # Step 5: Activate with forged rows + self-created capability
        store = TelosStore(org)
        try:
            store.activate_revision(digest, grant_id=forged_grt, capability=attacker_cap)
            active = store.get_active_digest()
            if active == digest:
                print("BYPASS: self-authorization succeeded via public APIs")
                sys.exit(1)
            else:
                print(f"BLOCKED: activation produced wrong digest: {{active}}")
                sys.exit(0)
        except TelosStoreError as e:
            print(f"BLOCKED at activate_revision: {{e}}")
            sys.exit(0)
        except Exception as e:
            print(f"BLOCKED at activate_revision: {{type(e).__name__}}: {{e}}")
            sys.exit(0)
    """))

    # Run attacker in subprocess (simulates model's execution context)
    import os as _os
    project_root = str(Path(__file__).resolve().parent.parent.parent.parent)
    env = dict(_os.environ)
    existing_pp = env.get("PYTHONPATH", "")
    env["PYTHONPATH"] = f"{project_root}:{existing_pp}" if existing_pp else project_root
    # Suppress HERMES_HOME precedence warnings in subprocess
    ...
    # Attacker must NOT print BYPASS
    if "BYPASS" in stdout:
        pytest.fail(
            f"Self-authorization bypass via public APIs succeeded!\n"
            f"stdout: {stdout}\nstderr: {stderr}"
        )

    # Must have been blocked at some step
    assert "BLOCKED" in stdout, (
        f"Expected BLOCKED output from attacker, got:\n"
        f"stdout: {stdout}\nstderr: {stderr}\n"
        f"returncode: {result.returncode}"
    )

    # Verify no mutation on the real organism
    assert tstore.get_active_digest() is None, (
        "Organism was mutated by attacker subprocess!"
    )
