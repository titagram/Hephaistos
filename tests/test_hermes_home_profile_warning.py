...
profile is active, get_hermes_home() should:
  1. STILL return ~/.hades (raising would brick 30+ module-level callers)
...
class TestGetHermesHomeProfileWarning:
    def test_classic_mode_no_active_profile_no_warning(
        ...
        """Classic mode: no active_profile file → silent, returns ~/.hades."""
    ...
    def test_hermes_home_set_suppresses_warning(
        ...
        profile_dir = tmp_path / ".hades" / "profiles" / "coder"
        profile_dir.mkdir(parents=True)
        (tmp_path / ".hades" / "active_profile").write_text("coder\n")
