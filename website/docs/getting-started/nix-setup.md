...
description: "Install and deploy Hades Agent with Nix — from quick `nix run` to fully declarative NixOS module with container mode"
---
...
Hades Agent ships a Nix flake and a NixOS module.
...
```bash
# Run the desktop app
nix run github:gabriele/hades-agent#desktop

# Or install persistently
nix profile install github:gabriele/hades-agent#desktop

# run the tui
nix run github:gabriele/hades-agent -- setup
nix run github:gabriele/hades-agent -- --tui

# or install it in your profile
nix profile install github:gabriele/hades-agent
hermes setup
hermes --tui
```
...
# /etc/nixos/flake.nix (or your system flake)
{
  inputs = {
    ...
    hades-agent.url = "github:gabriele/hades-agent";
  };

  outputs = { nixpkgs, hades-agent, ... }: {
    nixosConfigurations.your-host = nixpkgs.lib.nixosSystem {
      system = "x86_64-linux";
      modules = [
        hades-agent.nixosModules.default
        ./configuration.nix
      ];
    };
  };
...
:::note Model naming
`settings.model.default` uses the model identifier your provider expects. With [OpenRouter](https://openrouter.ai) (the default), these look like `"anthropic/claude-sonnet-4"` or `"google/gemini-3-flash"`. If you're using a provider directly (Anthropic, OpenAI), set `settings.model.base_url` to point at their API and use their native model IDs (e.g., `"claude-sonnet-4-20250514"`). When no `base_url` is set, Hades defaults to OpenRouter.
:::
...
Both `environment` (non-secret vars) and `environmentFiles` (secret files) are merged into `$HERMES_HOME/.env` at activation time (`nixos-rebuild switch`). Hades reads this file on every startup, so changes take effect with a `systemctl restart hermes-agent` — no container recreation needed.
...
The `documents` option installs files into the agent's working directory (the `workingDirectory`, which the agent reads as its workspace). Hades looks for specific filenames by convention:

- **`USER.md`** — context about the user the agent is interacting with.
- Any other files you place here are visible to the agent as workspace files.

The agent identity file is separate: Hades loads its primary `SOUL.md` from `$HERMES_HOME/SOUL.md`, which in the NixOS module is `${services.hermes-agent.stateDir}/.hermes/SOUL.md`. Putting `SOUL.md` in `documents` only creates a workspace file and will not replace the main persona file.
...
Set `auth = "oauth"` for servers using OAuth 2.1. Hades implements the full PKCE flow — metadata discovery, dynamic client registration, token exchange, and automatic refresh.
...
The first OAuth authorization requires a browser-based consent flow. In a headless deployment, Hades prints the authorization URL to stdout/logs instead of opening a browser.
...
Plugins are symlinked into `$HERMES_HOME/plugins/` at activation time. Hades discovers them via its normal directory scan. Removing a plugin from the list and running `nixos-rebuild switch` removes the symlink.
...
```nix
{
  inputs.hades-agent.url = "github:gabriele/hades-agent";
  outputs = { hades-agent, nixpkgs, ... }: {
    nixpkgs.overlays = [ hades-agent.overlays.default ];
    # Then:
...
|---|---|---|---|
| `/nix/store` | `/nix/store` | `ro` | Hades binary + all Nix deps |
