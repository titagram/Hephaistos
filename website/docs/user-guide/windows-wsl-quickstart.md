...
- You're doing POSIX-heavy development work and want your Hades sessions to share the same filesystem / paths as your dev tools.
- You already have a WSL2 environment and don't want to maintain a second install.

**When native is fine (or better):**
- Interactive chat, gateway (Telegram/Discord/etc.), cron scheduler, browser tool, MCP servers, and most Hades features all run natively on Windows.
...
This guide covers the parts of that split that specifically affect Hades: installing WSL2, getting files back and forth between Windows and Linux, networking in both directions, and the pitfalls people actually hit.
...
The native Windows install runs in Windows directly: your Windows terminal (PowerShell, Windows Terminal, etc.), Windows filesystem paths (`C:\Users\…`), and Windows processes.  Hades uses Git Bash to run shell commands, which is how Claude Code and other agents handle Windows today — it sidesteps the POSIX-vs-Windows gap without a full rewrite.

WSL2 runs a real Linux kernel in a lightweight VM, so Hades inside it is essentially identical to running on Ubuntu.  That's valuable when you want a real POSIX environment: `fork`, `/tmp`, UNIX sockets, signal semantics, PTY-backed terminals, shells like `bash`/`zsh`, and tools like `rg`, `git`, `ffmpeg` that behave the way they do on Linux.
...
Hades does not work reliably on WSL1 — WSL1 translates Linux syscalls on the fly and some behaviors (procfs, signals, network) diverge from real Linux.
...
The Hades gateway (and anything else you want to keep running) is easier to manage with systemd. On modern WSL, enable it once inside your distro:
...
### Where to put Hades and your projects

**Rule of thumb: keep everything Linux-ish inside the Linux filesystem.**

- Your Hades install (`~/.hermes/`) — Linux side. The installer already does this.
...
Clone inside WSL. Always, unless you have a specific reason not to. A typical Hades workflow (`hades chat`, tool calls that `rg`/`ripgrep` the repo, file watchers, background gateway) will be dramatically faster and more reliable against `~/code/myrepo` than `/mnt/c/Users/you/myrepo`.

One exception: **MCP bridges that launch Windows binaries.** If you're using `chrome-devtools-mcp` through `cmd.exe` (see [MCP guide: WSL → Windows Chrome](/guides/use-mcp-with-hermes#wsl2-bridge-hermes-in-wsl-to-windows-chrome)), Windows may complain with a `UNC` warning if Hades' current working directory is `~`. In that case, start Hades from somewhere under `/mnt/c/` so the Windows process has a drive-letter cwd.
...
### Case 1 — Hades in WSL talks to a service on Windows

Most common: you're running **Ollama, LM Studio, or a llama-server on Windows**, and Hades (inside WSL) needs to hit it.
...
- Using the **OpenAI-compatible API server** (exposed by `hades gateway` when `API_SERVER_ENABLED=true`) from a Windows-side tool. See the [API Server feature page](/user-guide/features/api-server).
...
On **NAT mode** (Windows 10 / older Windows 11), the default "localhost forwarding" in WSL2 will generally forward Linux-side `127.0.0.1` binds to Windows `localhost`, so a Hades service started with `--host 127.0.0.1` is usually reachable as `http://localhost:PORT` from Windows. If it isn't:
...
2. **Port-forward Windows → WSL VM.** In mirrored mode this is automatic. In NAT mode you have to do it yourself, per port, in Admin PowerShell:
   ...
   New-NetFirewallRule -DisplayName "Hades WSL 8080" `
...
## Running Hades services long-term on Windows

The Hades [Tool Gateway](/user-guide/features/tool-gateway) and the API server are long-lived processes. In WSL2 you have a few options for keeping them up.

### Desktop shortcut for opening Hades quickly

If you just want a double-click launcher for an interactive Hades shell, create
...
2. For the target, use your distro name (replace `Ubuntu` if needed):
   ...
   ```text
   wt.exe -w 0 -p "Ubuntu" wsl.exe -d Ubuntu --cd ~ -- bash -ic "hades"
   ```

3. Name it something obvious like `Hades`.

That opens Windows Terminal, starts your WSL distro, drops you in your Linux
home directory, and launches Hades. If `hades` is not on PATH yet, open WSL
once manually and run `source ~/.bashrc`, or replace the command with
`uv run hades` inside your project checkout.
...
If you enabled systemd per the setup section above, `hades gateway` and the API server work the way they do on any Linux machine. Use the gateway setup wizard:
...
AMD ROCm and Intel Arc support inside WSL2 is still evolving and outside Hades' test matrix — it may work with current drivers but we don't have a recipe to recommend.
...
**Massive slowness on `git status` / `hades chat` in a repo.**
...
Hades' cwd is inside the Linux filesystem, and Windows `cmd.exe` doesn't know what to do with it. Start Hades from `/mnt/c/...` for that session, or use a wrapper that `cd`s to a Windows-reachable path before invoking the Windows executable.
...
**`hades` not found after running the installer.**
