...
| Hades no-codebase diagnosis | `tests/agent/test_hades_bug_diagnosis_no_codebase.py`; focused Hades provider/client pytest | Review fixture coverage before claiming source-free diagnosis quality; confirm no raw source/file/shell tool violations | Report must show 5 complete bug fixtures, 2 insufficient-evidence fixtures, 100% evidence/freshness/awareness/tool/persistence coverage, and zero no-codebase violations |
