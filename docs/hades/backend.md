...
bodies or arguments.
Python and Node/TypeScript graph artifacts also include a metadata-only `logs`
map for logging calls, storing level/logger/context/path/line and a redacted
message hash rather than the message template.
