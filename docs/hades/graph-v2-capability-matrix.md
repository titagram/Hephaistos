...
| Cache reads and writes | Supported | The canonical `Illuminate\\Support\\Facades\\Cache` facade (or an exact import alias); only a static, non-secret, non-path key is public. Ordinary dot-prefixed keys (for example `.well-known`) are allowed. The sensitive `.env`/`.env.*` family and case variants are rejected, except exact lowercase `.env.example`. |
| Storage reads and writes | Supported | The canonical `Illuminate\\Support\\Facades\\Storage` facade (or an exact import alias); only a static, relative, sanitised path is public. Public dot segments such as `.well-known` are allowed; traversal (`.`/`..`) and sensitive names (`.env`/`.env.*`, `.ssh`, `.git`, `.aws`, case-insensitive) are not. Exact lowercase `.env.example` remains the documented public-template exception. |
