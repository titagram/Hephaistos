...
## Esecuzione Laravel Blade authorization route-param graph Hades - 2026-07-08

Stato: completata una tranche locale P0-4 per collegare authorization Blade ai
route param noti quando la convenzione view-route e' verificabile.

Integrazione locale:

- `hades.php_graph.v1` aggiunge edge `blade_authorization_route_param` quando
  una view `orders.show` contiene una direttiva authorization su subject
  `$order` e la route omonima `orders.show` dichiara `{order}`.
- L'edge collega `ability:<name>` a `route_param:<route>.<param>` e salva solo
  ability, helper, subject variable name, route name/param, path e line.
- Questo crea un ponte source-free tra UI authorization, route param e gli edge
  route model binding gia' prodotti dal controller PHP.
- Il parser non risolve type, non legge espressioni dinamiche e non conserva
  template/source raw.

Resta fuori da questa tranche:

- Risoluzione del subject in view non omonime alla route, `@include` con dati
  passati dal parent, variabili alias, component props e collegamento diretto
  alla policy class.
...
## Esecuzione Laravel Blade authorization model/policy graph Hades - 2026-07-08

Stato: completata una tranche locale P0-4 per collegare authorization Blade a
model e policy method quando il route model binding e' noto.

Integrazione locale:

- `populate_backend_ast` costruisce una mappa metadata-only `route -> param ->
  model/table` dai parametri tipizzati dei controller e dalle route gia'
  indicizzate.
- `hades.php_graph.v1` aggiunge `blade_authorization_model` quando un subject
  Blade coincide con un route param che ha model binding tipizzato.
- Aggiunge `blade_authorization_policy_method` quando quel model ha una policy
  mappata e il metodo policy per l'ability e' gia' indicizzato.
- Gli edge salvano solo ability, helper, subject, route param, model/table,
  policy class, path e line; non conservano template/source raw.

Resta fuori da questa tranche:

- Risoluzione di subject passati via include/component props, view non omonime
  alla route, policy dinamiche, `Gate::guessPolicyNamesUsing` e policy methods
  non corrispondenti direttamente al nome ability.
...
- Propagazione di `@includeWhen` data complessi, `@each`, component props e
  alias multipli tra parent/partial.
...
## Esecuzione Laravel Blade partial authorization include graph Hades - 2026-07-08

Stato: completata una tranche locale P0-4 per collegare authorization nei
partial Blade al contesto passato dal parent.

Integrazione locale:

- Post-processing metadata-only sugli edge `blade_include_data`,
  `blade_include_data_route_param` e `blade_authorization`.
- Aggiunge `blade_authorization_include_data` quando un partial autorizza
  `$order` e il parent passa `['order' => $order]` a quel partial.
- Aggiunge `blade_authorization_include_route_param` quando quel data slot e'
  gia' collegato a un route param del parent.
- Gli edge salvano solo ability, helper, subject, included view, data key,
  parent view, route param, path e line del partial authorization.
...
## Esecuzione Laravel Blade component prop model graph Hades - 2026-07-08

Stato: completata una tranche locale P0-4 per collegare props Blade component a
model/table quando il route param e' tipizzato.

Integrazione locale:

- `hades.php_graph.v1` aggiunge `blade_component_prop_model` per props bound
  safe in view route-named quando la source variable coincide con route param
  con model binding.
- Aggiunge `blade_component_prop_include_model` per props dentro partial quando
  `blade_component_prop_include_route_param` collega il prop al route param
  parent.
- Gli edge salvano solo component, prop, source variable, included/parent view
  quando presente, route name/param, model/table, path e line.
- Non conserva template/source raw.

Resta fuori da questa tranche:

- Component class resolution, props statiche non variabile, alias multipli,
  `@props`, nested components e data-flow interno.
...
## Esecuzione Laravel Blade component constructor graph Hades - 2026-07-08

Stato: completata una tranche locale P0-4 per collegare prop Blade bound ai
parametri del costruttore del class component.

Integrazione locale:

- L'indice class component salva metadata-only dei parametri `__construct()`
  per classi `App\\View\\Components`.
- `hades.php_graph.v1` aggiunge `blade_component_prop_class_param` quando un
  prop come `:order="$order"` coincide con il parametro `$order` del component.
- Se il parametro e' tipizzato su un model gia' mappato, l'edge salva anche
  `model` e `table`.
- Il fallback locale di `hades_backend_graph_search` mostra
  `component_param` e `component_param_type` nei summary.
- Non conserva signature completa, default value, template/source raw o corpo
  del component.

Resta fuori da questa tranche:

- Data-flow interno al component, trasformazioni nel costruttore, props
  statiche, alias registrati via service provider e parametri dinamici.
...
## Esecuzione Laravel Blade component registered alias graph Hades - 2026-07-08

Stato: completata una tranche locale P0-4 per risolvere alias component Blade
registrati esplicitamente.

Integrazione locale:

- L'indice class component legge chiamate `Blade::component(...)` con argomenti
  class const + alias literal, in entrambi gli ordini supportati.
- `hades.php_graph.v1` collega tag come `<x-orders-card>` alla classe
  `App\\View\\Components\\Orders\\Card` anche quando l'alias non deriva dalla
  convenzione path.
- Gli edge `blade_component_class` e `blade_component_prop_class_param`
  includono `component_alias_source=blade_component_registration` e path/line
  della registrazione.
- Il fallback locale di `hades_backend_graph_search` rende cercabili alias,
  classe, parametro e provenance di registrazione.
- Non conserva argomenti raw, signature completa, template/source raw o corpi
  dei provider.

Resta fuori da questa tranche:

- `Blade::componentNamespace`, prefix package, alias dinamici, componenti
  anonimi package-scoped e registrazioni costruite con variabili.
...
## Esecuzione Laravel Blade component template data-flow Hades - 2026-07-08

Stato: completata una tranche locale P0-4 per collegare variabili template dei
class component ai parametri e campi model noti.

Integrazione locale:

- Nei template `resources/views/components/*`, `hades.php_graph.v1` riconosce
  solo accessi safe `$param` e `$param->field`.
- Aggiunge `blade_component_template_param` da `component:*` al parametro del
  class component quando il nome coincide con `__construct()`.
- Aggiunge `blade_component_template_model_field` dal parametro alla colonna
  model/table quando il parametro e' tipizzato su un model gia' mappato.
- Il fallback locale di `hades_backend_graph_search` mostra
  `template_variable` e `template_field` nei summary.
- Non conserva espressioni Blade, valori, template/source raw o corpo del
  component.

Resta fuori da questa tranche:

- Espressioni complesse, method call nel template, loop/condition data-flow,
  slot/scoped slot, computed properties e trasformazioni interne.
