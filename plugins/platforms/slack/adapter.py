...
class SlackAdapter(BasePlatformAdapter):
        ...
        try:
            ...
            if "im:history" in granted and "mpim:history" not in granted:
                ...
                logger.warning(
                    ...
                    "Regenerating the app from `hades slack` produces a "
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: Hades handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Hades session — matching the per-thread behavior channels already
    ...
    def _write_slack_manifest_and_instruct() -> None:
        ...
        try:
            ...
            manifest = _build_full_manifest(
                bot_name="Hades",
                bot_description="Your Hades agent on Slack",
            )
            ...
            print_info(
                "   Re-run `hades slack manifest --write` anytime to refresh after "
                "Hades adds new commands."
            )
        except Exception as e:
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `hades update`)",
                True,
            ):
