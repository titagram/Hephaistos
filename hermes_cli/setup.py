"""
Interactive setup wizard for Hades Agent.
...
Config files are stored in ~/.hades/ for easy access.
"""
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("♇ Hades Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Hades using environment variables or config commands:")
    print_info("  hades config set model.provider custom")
    print_info("  hades config set model.base_url http://localhost:8080/v1")
    print_info("  hades config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'hades setup' in an interactive terminal to use the full wizard.")
    print()
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'hades setup')")
...
def _print_setup_summary(config: dict, hermes_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'hades setup' to configure"))
    ...
    # Video generation — opt-in via `hades tools` → Video Generation.
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'hades setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'hades setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'hades setup terminal'"))
    ...
    # Spotify (OAuth via hades auth spotify — check auth.json, not env vars)
    try:
    ...
    print()
    print(f"   {color('hades setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('hades setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('hades setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('hades setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('hades setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('hades config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('hades config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('hades config set <key> <value>', Colors.GREEN)}")
    ...
    print(color("─" * 60, Colors.DIM))
    print()
    ready_to_chat = _model_section_has_credentials(config)
    if ready_to_chat:
        print(color("🚀 Ready to go!", Colors.CYAN, Colors.BOLD))
    else:
        print(
            color(
                "🚀 Setup saved. Configure a model/provider before chatting.",
                Colors.CYAN,
                Colors.BOLD,
            )
        )
    print()
    if ready_to_chat:
        print(f"   {color('hades', Colors.GREEN)}              Start chatting")
    else:
        print(f"   {color('hades setup model', Colors.GREEN)}        Configure model/provider")
    print(f"   {color('hades gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('hades doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `hades tools` and `hades setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``hades model``)
    ...
    provider added to ``hades model`` is automatically available here.
    ...
    # Delegate to the shared Hades model flow — handles provider picker,
    ...
    try:
        ...
        print_info("You can try again later with: hades model")
    ...
    # on demand via `hades auth add`, `hades setup` vision, and
    # `hades setup tts`. This keeps both quick and full setup thin.
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``hades model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            from hermes_constants import display_hermes_home as _dhh
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence "
                f"until removed from {_dhh()}/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the Hades auth store) > existing XAI_API_KEY > prompt the user.
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'hades setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Hades runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `hades setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `hades setup terminal`.
...
def _apply_default_agent_settings(config: dict):
    ...
    print_info("  Run `hades setup agent` later to customize.")
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Hades delivers cron job results,")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Hades to iMessage via BlueBubbles — a free, open-source")
...
def _setup_webhooks():
    ...
    print()
    print_info("   Open config in your editor:  hades config edit")
    print_info("   Open config in your editor:  hades config edit")
...
def setup_gateway(config: dict):
    ...
    print_info("Connect to messaging platforms to chat with Hades from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'hades setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     hades config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                try:
                    ...
                    print_info("  You can try manually: hades gateway install")
            else:
                print_info("  You can install later: hades gateway install")
                if supports_systemd and os.geteuid() == 0:  # windows-footgun: ok — guarded by supports_systemd (Linux only)
                    print_info("  Or as a boot-time service: hades gateway install --system")
            ...
            if is_container():
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   hades gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `hades setup tools` and `hades tools` use the same flow:
...
# Config values may have different semantics between OpenClaw and Hades.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Hades to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Hades at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Hades at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Hades at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Hades at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Hades equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Hades config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Hades.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Hades's yolo mode.", Colors.YELLOW))
...
def _offer_openclaw_migration(hermes_home: Path) -> bool:
    ...
    print_info("Hades can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: hades claw migrate --dry-run"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: hades claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Hades configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Hades config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Hades (use hades claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``hades setup --portal`` and ``hades portal``. This is the
    ...
    Hades session — model selected, provider set, and web/image/tts/browser
    tools routed via their Portal sub — without being told to run
    ``hades setup`` and hunt for the quick-setup option.
    ...
    (``_run_first_time_quick_setup``) and the same one ``hades model`` runs
    when you pick Nous. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``hades portal`` always offers a model picker,
    ...
    print(color("│     ♇ Hades Setup — Nous Portal (one-shot)              │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    # setup calls, so `hades portal` == quick setup's Nous step.
    try:
        ...
        print_info("  You can retry later with `hades portal`.")
        return
    except Exception as exc:
        logger.debug("_model_flow_nous error during `hades portal`: %s", exc)
        print()
    ...
    print_info("  Run `hades portal info` to inspect routing.")
    print_info("  Run `hades` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      hades setup           — full or quick (auto-detected)
      hades setup model     — just model/provider
      hades setup tts       — just text-to-speech
      hades setup terminal  — just terminal backend
      hades setup gateway   — just messaging platforms
      hades setup tools     — just tool configuration
      hades setup agent     — just agent settings
    """
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ♇ Hades Setup — {label:<35s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ♇ Hades Agent Setup Wizard                 │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Hades Agent installation.        │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have Hades configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'hades setup model|terminal|")
    ...
    print()
    print_info("You can edit these files directly or use 'hades config edit'")
    ...
    # Tune later with `hades setup agent`.
    if not is_existing:
...
def _run_first_time_quick_setup(config: dict, hermes_home, is_existing: bool):
    ...
    settings, tools); the user can customize later via ``hades setup <section>``
    or switch providers with ``hades model``.
    """
    from hermes_cli.config import load_config
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'hades setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    hades setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  hades setup gateway")
    print()
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``hades tools`` (which
...
def _run_blank_slate_setup(config: dict, hermes_home, is_existing: bool):
    ...
    if path == 0:
        ...
        # `hades update` runs don't re-inject them.
        try:
        ...
        print_info("  Enable tools:        hades tools")
        print_info("  Seed skills:         hades skills opt-in --sync")
        print_info("  Add MCP servers:     hades mcp add")
        print_info("  Enable plugins:      hades plugins")
        print_info("  Tune agent settings: hades setup agent")
        print()
        _print_setup_summary(config, hermes_home)
        return
...
def _blank_slate_walkthrough(config: dict, hermes_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            print_info("`hades update` runs from re-injecting them. Opt back in any")
            print_info("time with `hades skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `hades tools`.")
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `hades plugins list` / `hades plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `hades plugins`.")

    # ── MCP servers (off unless chosen) ──
    print()
    print_header("MCP Servers")
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `hades mcp add <name> --url ... | --command ...`.")
    else:
        print_info("No MCP servers configured. Add later with `hades mcp add`.")
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   hades tools")
    print_info("  Seed skills:         hades skills opt-in --sync")
    print_info("  Add MCP servers:     hades mcp add")
    print_info("  Tune agent settings: hades setup agent")
    print()
...
def _run_quick_setup(config: dict, hermes_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'hades setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Hades to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'hades setup gateway'.")
