...
def _ensure_tui_node() -> None:
    ...
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "HADES_HOME": hermes_home, "HERMES_HOME": hermes_home},
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
...
def cmd_whatsapp(args):
    ...
    if (session_dir / "creds.json").exists():
        ...
        print("⚠ Pairing may not have completed. Run 'hades whatsapp' to try again.")
...
def select_provider_and_model(args=None):
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'hades model' for "
                "available providers, or run 'hades doctor' to diagnose config "
                "issues."
            )
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `hades setup` to configure {pconfig.name} again."
        )
        return "", True
...
def cmd_slack(args):
    ...
    if sub in {None, ""}:
        ...
        print(
            ...
            "\n"
            "Run `hades slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        candidates = [
            *release_dir.glob("mac*/Hades.app/Contents/MacOS/Hades"),
            *release_dir.glob("mac*/Hermes.app/Contents/MacOS/Hermes"),
        ]
    elif sys.platform == "win32":
        candidates = [
            release_dir / arch / name
            for arch in ("win-unpacked", "win-ia32-unpacked", "win-arm64-unpacked")
            for name in ("Hades.exe", "Hermes.exe")
        ]
    else:
        candidates = [
            release_dir / arch / name
            for arch in ("linux-unpacked", "linux-arm64-unpacked")
            for name in ("hades", "Hades", "hermes", "Hermes")
        ]
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ Hades Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
...
def cmd_gui(args: argparse.Namespace):
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            if build_result.returncode != 0:
                ...
                print("    ELECTRON_MIRROR=<mirror-base-url> hades desktop --force-build")
    ...
    print(f"→ Launching packaged Hades Desktop: {' '.join(launch_command)}")
...
def _kill_stale_dashboard_processes(
    ...
    if killed:
        ...
        print("    hades dashboard --port <port>")
...
def _format_concurrent_instances_message(
    ...
    lines.append("")
    lines.append("  Close Hades Desktop, exit any open `hermes` REPLs, and")
...
def _quarantine_running_hermes_exe(
    ...
    for shim in _hermes_exe_shims(scripts_dir):
        ...
        print(
            "    Close Hades Desktop, exit other `hermes` REPLs, stop the "
...
def _refresh_active_lazy_features() -> None:
    ...
    if failed:
        ...
        print("  `hades update` once the upstream issue is resolved.")
...
def _verify_console_scripts_installed(
    ...
    *,
    ...
    try:
        ...
        print(
            "  ⚠ Entry point repair failed; try `hades update --force` after "
            "closing other Hades processes."
        )
        return
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `hades update` output above.")
        return
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        if version_bump_only:
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: hades config migrate")
            else:
                print()
                print("Skipped. Run 'hades config migrate' later to configure.")
        else:
        ...
        print("  hades model               # Select provider and model")
...
def _report_dashboard_status() -> int:
    ...
    if not pids:
        print("No Hades dashboard processes running.")
        return 0

    print(f"{len(pids)} Hades dashboard process(es) running:")
    for pid in pids:
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    print("    [2] OAuth via Nous Portal (run `hades dashboard register`)")
    print("    [3] Cancel")
    print()
...
def main():
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `hades model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
