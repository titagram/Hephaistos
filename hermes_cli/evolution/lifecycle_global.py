...
from .organism_identity import OrganismIdentityError, create_organism_identity, load_organism_identity
...
def ensure_global_lifecycle_initialized(
    ...
    # No identity — acquire the global lifecycle lock to serialize init
    from .locking import global_lifecycle_lock

    with global_lifecycle_lock(
        organism_root=root,
        timeout_seconds=120,
    ):
        ...
        if identity_path.exists():
            ...
            try:
                ...
                raise LifecycleInitError(
                    "organism_identity_lineage_mismatch"
                ) from None
            finally:
                ledger.connection.close()

        # Scan legacy profiles before fresh init
        ...
        coherent = [
            c
            for c in candidates
            if c.status == LegacyStatus.COHERENT_BASELINE_ONLY
        ]
        ...
        # Race-check: identity may have been created concurrently by another process
        if identity_path.exists():
            ident = load_organism_identity(root)
            if not ledger_path.exists():
                raise LifecycleInitError("organism_identity_without_ledger")
