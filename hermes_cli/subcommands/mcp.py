...
def build_mcp_parser(subparsers, *, cmd_mcp: Callable) -> None:
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Hades as an MCP server",
        description=(
            "Manage MCP server connections and run Hades as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'hades mcp add' to connect to a new server, or\n"
            "'hades mcp serve' to expose Hades conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Hades as an MCP server (expose conversations to other agents)",
    )
    ...
    mcp_sub.add_parser(
        "picker",
        help="Interactive catalog picker (also the default for `hades mcp`)",
    )
    mcp_sub.add_parser(
        "catalog",
    ...
    mcp_install_p = mcp_sub.add_parser(
        "install",
        help="Install a catalog MCP by name (e.g. `hades mcp install playwright`)",
    )
