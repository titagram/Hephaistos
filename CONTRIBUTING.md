...
Thank you for contributing to Hades Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want Hades to work everywhere.
...
- **Search both open *and* merged PRs and issues** for your topic or error symptom — the duplicate-check in the PR template fires at review time, after you've already done the work:
  ```bash
  gh search issues --repo gabriele/hades-agent "<your terms>"
  gh search prs --repo gabriele/hades-agent --state all "<your terms>"
  ```
  Or use the web UI: [issues](https://github.com/gabriele/hades-agent/issues?q=) · [PRs (all states)](https://github.com/gabriele/hades-agent/pulls?q=is%3Apr).
...
Related: #38284 covers the agent-side analog — Hades itself checking existing issues and PRs before deep self-troubleshooting. This section is the human-contributor complement.
...
Bundled skills (in `skills/`) ship with every Hades install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `hades skills browse` (labeled "official") and install it with `hades skills install` (no third-party warning, built-in trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Hades Agent Discord](https://discord.gg/hades-agent). Users can install it with `hades skills install`.
...
- Integrate with `hades memory setup` via `post_setup()` — no need to touch core code
...
The reason is maintenance load, not quality. Every external product absorbed into the core tree becomes ours to keep working against a fast-moving codebase, for a backend we don't own and can't control. Hades ships a lot and the core moves quickly; coupling third-party products into it creates an open-ended burden on the maintainers.

Publish these as a **standalone plugin repo** instead:

- Implement the relevant ABC and use the existing plugin discovery path (`~/.hermes/plugins/`, project `.hermes/plugins/`, or a pip entry point) — see [Build a Hades Plugin](https://hades-agent.local/docs/guides/build-a-hermes-plugin)
- Register lifecycle hooks (`pre_tool_call`, `post_tool_call`, `pre_llm_call`, `post_llm_call`, `on_session_start`, `on_session_end`), tools (`ctx.register_tool`), and CLI subcommands (`ctx.register_cli_command`) through the surface we already expose — no core changes needed
- If your plugin needs a capability the framework doesn't expose, that's a feature request to **widen the generic plugin surface** (a new hook or `ctx` method) — never special-case your plugin in core
- Promote it in the [Hades Agent Discord](https://discord.gg/hades-agent) `#plugins-skills-and-skins` channel so users can find and install it
...
The installer creates the Hades venv, wires the `hades` command, stamps the
install method for `hades update`, and clones the full git project into
`$HERMES_HOME/hades-agent` (usually `~/.hermes/hades-agent`). That keeps your
...
Use this only if you intentionally do not want Hades' managed install layout
(for example, a throwaway clone inside a container or CI job). If you install
this way, make sure you run the `hades` entrypoint from this venv; running the
system `python3 -m hermes_cli.main` can pick up unrelated system Python
packages.

```bash
git clone https://github.com/gabriele/hades-agent.git
cd hades-agent
...
```bash
# The standard installer already put `hades` on PATH.
hades doctor
hades chat -q "Hello"
```

If you used the manual clone fallback, run `./hades` from the checkout or
symlink this clone's venv explicitly:

```bash
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/hades" ~/.local/bin/hades
```
...
├── tests/                    # Test suite
├── website/                  # Documentation site (hades-agent.local)
│
...
The user may skip setup and keep loading the skill. Hades only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `hades setup` or update `~/.hermes/.env` locally.
...
2. **Tools referenced in SKILL.md prose must be native Hades tools or MCP servers the skill explicitly expects.** When the skill needs a capability, point at the proper tool by name in backticks: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, `` `patch` ``, `` `search_files` ``, `` `vision_analyze` ``, `` `browser_navigate` ``, `` `delegate_task` ``, `` `image_generate` ``, `` `text_to_speech` ``, `` `cronjob` ``, `` `memory` ``, `` `skill_view` ``, `` `todo` ``, `` `execute_code` ``.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "Hades Agent" is the secondary collaborator. If the contributor's commit shows "Hades Agent" as author because they used Hades to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Hades tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `hades --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Hades uses a data-driven skin system — no code changes needed to add a new skin.
...
Hades runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
Hades has terminal access. Security matters.
...
2. **Test manually**: Run `hades` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/gabriele/hades-agent/issues)
...
- **Discord**: [discord.gg/hades-agent](https://discord.gg/hades-agent) — for questions, showcasing projects, and sharing skills
